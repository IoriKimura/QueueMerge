...
public class MergeSort {
    ...
    {
        int step = 1; //+1, Шаг разбиения последовательности. Начинается с единицы.
        int[] temp = new int[size];//+1, Создаём дополнительный массив
        int left = 0;//+1, левая граница участка
        Main.nOp+=3;
       ...
       // ∑(step=1 -> n/2)(1 + 1 + 1 + 2 + 3 + 20.5n^3+71.5n^2+113n+62 + 4n^2+20n+18) = ∑(step=1 -> n/2)(88 + 20.5n^3 + 75.5n^2 + 133n) = 20.5n^3(n/2) + 75.5n^2(n/2) + 133n(n/2) + 88(n/2) = 10.25n^4+37.75n^3+66.5n^2+44n;
        while (step < size) { //+1
            int index = 0;//+1, индекс результирующего массива
            left = 0;//+1, всё также левая граница
            Main.nOp+=3;
            int midl = left + step; //+2, середина участка
            int right = left + step * 2; //+3, граница правого участка
            Main.nOp+=5;
            ...
            //Пока левая граница сортируемого участка в пределах последовательности
            while( left < size ) {//+1
                Main.nOp++;
                //Сортируемый участок не выходит за границы последовательности
                midl = Math.min(midl, size); //+3
